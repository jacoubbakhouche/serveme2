...
import { Input } from '@/components/ui/input'; // ✨ 1. استيراد حقل الإدخال
import { Label } from '@/components/ui/label';   // ✨ 2. استيراد الملصق
import { supabase } from '@/integrations/supabase/client';
import { useToast } from '@/hooks/use-toast';
import { Link, useNavigate } from 'react-router-dom';

// أيقونات جوجل وفيسبوك (تبقى كما هي)
const GoogleIcon = () => ( <svg className="ml-2 h-4 w-4" role="img" viewBox="0 0 24 24" xmlns="http://www.w.org/2000/svg"><title>Google</title><path fill="currentColor" d="M12.48 10.92v3.28h7.84c-.24 1.84-.85 3.18-1.73 4.1-1.02 1.02-2.62 1.9-4.82 1.9-5.78 0-10.47-4.88-10.47-10.92S6.7 1.08 12.48 1.08c3.24 0 5.4 1.35 6.67 2.53l-2.52 2.34c-.82-.76-2.04-1.35-4.15-1.35-4.82 0-8.72 3.9-8.72 8.72s3.9 8.72 8.72 8.72c5.33 0 8.14-3.83 8.4-7.42h-8.4v-3.28z"/></svg> );
const FacebookIcon = () => ( <svg className="ml-2 h-4 w-4" role="img" viewBox="0 0 24 24" xmlns="http://www.w.org/2000/svg"><title>Facebook</title><path fill="currentColor" d="M22.675 0H1.325C.593 0 0 .593 0 1.325v21.351C0 23.407.593 24 1.325 24H12.82v-9.294H9.692v-3.622h3.128V8.413c0-3.1 1.893-4.788 4.659-4.788 1.325 0 2.463.099 2.795.143v3.24l-1.918.001c-1.504 0-1.795.715-1.795 1.763v2.313h3.587l-.467 3.622h-3.12V24h6.116c.732 0 1.325-.593 1.325-1.325V1.325C24 .593 23.407 0 22.675 0z"/></svg> );
...
    // ✨ 3. إضافة حالات جديدة للبريد الإلكتروني وكلمة المرور
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    const [isSignUp, setIsSignUp] = useState(false); // للتبديل بين التسجيل والدخول
...
    }, [navigate, toast]);
    
    // ✨ 4. دالة جديدة للتعامل مع التسجيل/الدخول التقليدي
...
            }
            // onAuthStateChange سيتولى عملية التوجيه بعد الدخول
        }
        setLoading(false);
    };


    const handleGoogleLogin = async () => { /* ... (تبقى كما هي) */ setLoading(true); const { error } = await supabase.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: `${window.location.origin}/dashboard` } }); if (error) { toast({ title: "خطأ في تسجيل الدخول عبر جوجل", description: error.message, variant: "destructive" }); setLoading(false); } };
    const handleFacebookLogin = async () => { /* ... (تبقى كما هي) */ setLoading(true); const { error } = await supabase.auth.signInWithOAuth({ provider: 'facebook', options: { redirectTo: `${window.location.origin}/dashboard` } }); if (error) { toast({ title: "خطأ في تسجيل الدخول عبر فيسبوك", description: error.message, variant: "destructive" }); setLoading(false); } };
...
                    {/* ✨ 5. إضافة فورم الإيميل وكلمة المرور */}
