...
import { Send, BadgeCheck, Paperclip, X, Loader2, Mic, StopCircle, Plus, Trash2 } from 'lucide-react';
...
// --- 1. استيراد مكتبة Capacitor/Camera ---
...
const ProviderChat = ({ receiverId, receiverName, receiverAvatarUrl, receiverIsVerified }: ProviderChatProps) => {
  ...
  // --- 2. تعديل دالة startRecording بالكامل ---
  const startRecording = async () => {
    if (isRecording) return;
    try {
      // الخطوة أ: التحقق من الإذن وطلبه إذا لزم الأمر
      let permStatus = await Camera.checkPermissions();
      if (permStatus.microphone !== 'granted') {
        permStatus = await Camera.requestPermissions({ permissions: ['microphone'] });
      }

      // الخطوة ب: إذا رفض المستخدم الإذن، أظهر رسالة واخرج
      if (permStatus.microphone !== 'granted') {
        toast({
          title: "الإذن مرفوض",
          description: "للتسجيل الصوتي، يرجى تفعيل إذن الميكروفون من إعدادات التطبيق.",
          variant: "destructive",
        });
        return; // الخروج من الدالة
      }
      
      // الخطوة ج: الإذن ممنوح، يمكنك الآن تشغيل الكود الأصلي بأمان
      ...
      toast({
        title: "حدث خطأ",
        description: "لم نتمكن من الوصول إلى الميكروفون. قد تحتاج لإعادة تشغيل التطبيق.",
        variant: "destructive"
      });
    }
  };
